...
class random_forest(object):
    def __init__(self,task_type,task_name,target_model):
        ...
        if task_type=="source":
            ...
            if not os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                               'output{}/source/{}'.format(self.target_output_name,self.task_name))):
                os.makedirs(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                         'output{}/source/{}'.format(self.target_output_name,self.task_name)))
            self.model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                           'output{}/source/{}/random_forest.pickle'.format(self.target_output_name,self.task_name))

            # 特征重要性保存路径
            self.import_feature_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                    'output{}/source/{}/random_forest_feature_importance.txt'.format(self.target_output_name,self.task_name))
    ...
    def target_train(self):
        ...
        eta = min(origin_data.iloc[:, -1].values)  # eta是当前最优值
        ...
        for i in trange(self.confs['sample_params']['sample_epoch']):
            ...
            # 计算相似性列表
            self.similarity_list=self.cal_Similarity("./output{}/source/".format(self.target_output_name),origin_data)
            ...
            self.selected_svm_classifier = np.array(self.svm_classifier)[self.selected_svm_index-1]
    ...
    def get_one_clf(self,index):
        ...
        sample_data_2 = pd.read_csv("./output{}/source/sourcetask_{}/temp_data_real_perf_min.csv".format(self.target_output_name,index))
        sample_data = pd.concat([sample_data_1, sample_data_2], axis=0)
        sample_data = sample_data.reset_index(drop=True)
    ...
    def vis_cls(self,selected_svm_index, svm_classifier, vis_cls_path, confs, iter,condenseed_sample_data,discard_data,similarity_list):
        ...
        saved_x,saved_y=condenseed_sample_data.iloc[:,0].values,condenseed_sample_data.iloc[:,1].values
        ...
        for i in range(len(svm_classifier)):
            ...
            zz=cls.predict(list(zip(x1,y1,[0]*10000)))
            z1=zz.reshape(100,100)
            plt.imshow(z1, extent=[-10, 10, -10, 10], origin='lower',  alpha=0.4, cmap='Blues')
            plt.colorbar(label='classifier')
            plt.scatter(saved_x,saved_y,marker='o',alpha=0.2, s=2,c="green")
            # plt.scatter(discard_x,discard_y,marker='*',alpha=0.2,s=8,c="green")
            plt.title("Similarity:{ff:.2f}".format(ff=similarity_list[i]))

            if i+1 in selected_svm_index:
