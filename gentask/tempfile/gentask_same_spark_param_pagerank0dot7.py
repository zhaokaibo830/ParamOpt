...
def get_performance(params):
    ...
    req1 = request.Request('http://192.168.0.168:9002/api/spark/runPagerankForTime?%s' % value, headers=headers)  # 这样就能把参数带过去了
...
def get_sample(path, sample_num):
    ...
    temp_filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'taskdata/sourcetask_pagerank_0dot7.csv')
