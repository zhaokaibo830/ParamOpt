...
# 随机采样
def random_sample(selected_params, sample_num, unimportant_feature_list):
    """
    对不太重要的参数进行随机采样
...
# 尽量全覆盖采样
def sobel_sample(selected_params, important_feature_list,important_feature_sample_step):
    """

    :param selected_params:
    :param important_feature_list:
    :param important_feature_sample_step:
    :return:
    """
    sample_num=0
    for key in selected_params.keys():
        if key in important_feature_list:
            sample_num += (selected_params[key][1][1] - selected_params[key][1][0])/important_feature_sample_step
...
def get_performance(params):
    ...
    req1 = request.Request('http://47.104.101.50:8088/x264/exec?%s' % value, headers=headers)  # 这样就能把参数带过去了
    print('http://47.104.101.50:8088/x264/exec?%s' % value)
    # 下面是获得响应
    i = 0
