...
class random_forest(object):
    def __init__(self,task_type,task_name,target_model):
        ...
        if task_type=="source":
            ...
            if not os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                               'source_output/{}'.format(self.task_name))):
                os.makedirs(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                         'source_output/{}'.format(self.task_name)))
            self.model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                           'source_output/{}/random_forest.pickle'.format(self.task_name))

            # 特征重要性保存路径
            self.import_feature_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                    'source_output/{}/random_forest_feature_importance.txt'.format(self.task_name))
            ...
            self.temp_filename = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                          'source_output/{}/temp.csv'.format(self.task_name))
        else:
            if not os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)),
    ...
    def target_train(self):
        ...
        important_feature_sample_step=self.confs['sample_params']['important_feature_sample_step']  # 重要特征的初始采样步长
        sample_step_change=self.confs['sample_params']['sample_step_change']  # 采样步长每次变为原来的多少
        ...
        for i in trange(self.confs['sample_params']['sample_epoch']):
            ...
            # 计算相似性列表
            self.similarity_list=self.cal_Similarity("./source_output/",origin_data)
            # print("_______")
            # print(self.similarity_list)

            # 选择原任务,得到原任务对应的索引
            self.selected_svm_index=self.choose_source(self.slected_k_source,self.similarity_list,self.sourcetask_num)
            ...
            # 更新重要特征的采样步长
            important_feature_sample_step = important_feature_sample_step * sample_step_change

            # 获取压缩后得到采样数据
            condenseed_sample_data=self.condense_sampledata(self.selected_svm_classifier,sample_data,self.slected_k_source)

            # 把此轮剩余的样本数加到every_epoch_remain_sample_num末尾
            every_epoch_remain_sample_num.append(len(condenseed_sample_data)/len(sample_data))
            ...
            writer.add_scalar('pre_min', predict[max_index], global_step=i)

        temp_data.to_csv(self.temp_filename, index=False)
        print(every_epoch_remain_sample_num)
        np.savetxt('every_epoch_remain_sample_ratio.txt', every_epoch_remain_sample_num)
    ...
    def cal_Similarity(self,sourcetask_models_path, origin_data):
        ...
        similarity_list = [0]*self.sourcetask_num
        thread_pool = ThreadPoolExecutor(max_workers=self.sourcetask_num)
        for i,sourcetask_model in enumerate(self.sourcetask_models):
            similarity_list[i]=thread_pool.submit(self.cal_one_Similarity,sourcetask_model,target_X,target_Y).result()
    ...
    def cal_one_Similarity(self,sourcetask_model,target_X,target_Y):
        ...
        temp=np.logical_xor(source_M_col<source_M_row,target_Y_col<target_Y_row)
        rank_loss=np.sum(temp[np.triu_indices(len(target_Y))])
    ...
    def choose_source(self,slected_k_source,similarity_list,sourcetask_num):
        """
        选择k个原任务
        ...
        selected_svm_index=np.random.choice(sourcetask_num,slected_k_source,replace=False,p=p_list)+1
    ...
    def get_one_clf(self,index):
        ...
        sample_data_2 = pd.read_csv("./source_output/sourcetask_{}/temp.csv".format(index))
        sample_data = pd.concat([sample_data_1, sample_data_2], axis=0)
        sample_data = sample_data.reset_index(drop=True)
