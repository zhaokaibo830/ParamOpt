...
class random_forest(object):
    def __init__(self,task_name,target_model):
        ...
        # 中间结果数据集路径
        self.temp_filesname = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                          'target_output/{}/'.format(self.task_name))
    ...
    def target_train(self):
        ...
        for i in range(self.confs['sample_params']['sample_epoch']):
            ...
            # print(perf)
            writer.add_scalar('perf_min_forest', predict[max_index], global_step=i)
        ...
        temp_data_real_perf_min.to_csv(self.temp_filesname+"temp_data_real_perf_min.csv", index=False)
        temp_data_forest_perf_min.to_csv(self.temp_filesname + "temp_data_forest_perf_min.csv", index=False)
