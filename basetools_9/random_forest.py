...
class random_forest(object):
    ...
    def target_train(self):
        ...
        for i in trange(self.confs['sample_params']['sample_epoch']):
            ...
            candidate_sample_data = pd.DataFrame(columns=list(self.selected_params.keys()))
            if self.candidate_supplement==0:
                if len(condenseed_sample_data) == 0:
                    condenseed_sample_data = sample_data.iloc[:2, :]
            elif self.candidate_supplement==1:
                condenseed_sample_data_length=len(condenseed_sample_data.iloc[:,:].values)
                if condenseed_sample_data_length<self.candidate_supplement_threshold:
                    candidate_sample_data=random_sample_1(self.selected_params,self.candidate_supplement_threshold-condenseed_sample_data_length)


            # 可视化分类器
            self.vis_cls(self.selected_svm_index,self.svm_classifier, self.vis_cls_path, self.confs, i + 1, condenseed_sample_data, discard_data,candidate_sample_data,self.similarity_list)
            ...
            # 计算mre
            mre_sum.append(mre_sum[-1]+np.fabs(perf-self.model.predict([sample_data.iloc[max_index, :]])[0])*100/np.fabs(perf))
    ...
    def vis_cls(self,selected_svm_index, svm_classifier, vis_cls_path, confs, iter,condenseed_sample_data,discard_data,candidate_sample_data,similarity_list):
        ...
        candidate_x,candidate_y=candidate_sample_data.iloc[:,0].values,candidate_sample_data.iloc[:,1].values
        ...
        for i in range(len(svm_classifier)):
            ...
            plt.scatter(candidate_x, candidate_y, marker='o', alpha=0.4, s=2, c="yellow",label="candidate-data")
