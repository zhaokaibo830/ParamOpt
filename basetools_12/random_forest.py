...
class random_forest(object):
    def __init__(self,task_type,task_name,target_model):
        ...
        if self.candidate_supplement==0:
            self.candidate_supplement_name="_cs0"
        elif self.candidate_supplement==1:
            self.candidate_supplement_name="_cs1"
        elif self.candidate_supplement==2:
            self.candidate_supplement_name="_cs2"
        elif self.candidate_supplement==3:
            self.candidate_supplement_name="_cs3"
    ...
    def target_train(self):
        ...
        # 获取源任务模型
        self.get_sourcetask_models("./output{}{}{}{}/source/".format(self.target_output_name,
                                                                   self.select_unimportant_parameter_samples_name,
                                                                     self.candidate_supplement_name,
                                                                       self.sourcetasks_get_method_name))

        # 获取初始样本在源任务模型上计算得到的值
        self.source_M_list=self.cal_M_list(self.sourcetask_models,origin_data.iloc[:,:-1].values)
        ...
        for i in trange(self.confs['sample_params']['sample_epoch']):
            ...
            # 获取原任务分类器
            self.selected_svm_classifier=self.get_selected_sourcetask_svm_clf(np.array(self.selected_svm_index)+1)
            ...
            if self.candidate_supplement==0:
                ...
                while len(candidate_data_temp) < self.candidate_supplement_threshold:
                    ...
                    min_distance=self.get_two_point_min_distance(candidate_data_temp)
                    sample_list=[]
                    for k in range(len(candidate_data_temp)):
                        sample_temp=self.box_of_point_sample(candidate_data_temp[k],min_distance)
            ...
            self.updata_M_list(self.source_M_list,self.sourcetask_models,condenseed_sample_data.iloc[max_index, :].values)
    ...
    def get_sourcetask_models(self, sourcetask_models_path):
        """
    ...
    def box_of_point_sample(self,center,min_distance):
        ...
        for j in range(len(center)):
            ...
            if all_params[key][0]=="int":
                if min_distance<1:
                    all_params[key][1][0],all_params[key][1][1]=center[j]-1,center[j]+1
                else:
                    all_params[key][1][0], all_params[key][1][1] = center[j] - int(min_distance), center[j] + int(min_distance)
            elif  all_params[key][0]=="float":
                all_params[key][1][0], all_params[key][1][1] = center[j] - min_distance, center[j] + min_distance
